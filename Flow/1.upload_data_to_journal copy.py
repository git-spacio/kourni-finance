...
def D_filtering_data(df_merged: pd.DataFrame) -> pd.DataFrame:
    ...
    - Fecha (transferDate en formato YYYY-MM-DD)
    ...
    # Convertir transferDate a datetime y formatear como YYYY-MM-DD
    df_filtered['transferDate'] = pd.to_datetime(df_filtered['transferDate']).dt.strftime('%Y-%m-%d')
    
    # Seleccionar y renombrar las columnas necesarias
    df_result = df_filtered[['transferDate', 'Etiqueta', 'Importe']].rename(
        columns={
            'transferDate': 'Fecha'
        }
    )
